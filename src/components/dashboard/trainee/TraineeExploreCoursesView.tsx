...
interface TraineeExploreCoursesViewProps {
  currentUser: User;
  onOpenCourse: (courseId: string) => void;
  onToast: (msg: string) => void;
}

export const TraineeExploreCoursesView: React.FC<TraineeExploreCoursesViewProps> = ({ currentUser, onOpenCourse, onToast }) => {
  ...
  return (
    <div className="space-y-6 text-left">
      <div className="bg-white rounded-3xl border border-[#E7E5E4] p-6 sm:p-8 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h2 className="text-2xl font-black text-[#0F172A] tracking-tight">Explore IMD Curriculum Catalogue</h2>
          <p className="text-xs sm:text-sm text-[#64748B] mt-1">
            Browse WMO-aligned courses in numerical weather prediction, radar meteorology, and climate science.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filtered.map((course) => (
          <div key={course.id} className="bg-white rounded-3xl border border-[#E7E5E4] overflow-hidden shadow-xs flex flex-col justify-between group">
            <div>
            ...
            <div className="p-6 pt-0 flex items-center justify-between border-t border-[#E7E5E4] mt-4">
              <span className="text-xs font-bold text-[#2A7F7E]">{course.durationHours} Hours • {course.level}</span>
              <button
                onClick={() => onOpenCourse(course.id)}
                className="px-4 py-2 rounded-xl bg-[#0D3B66] text-white text-xs font-semibold hover:bg-[#092b4d]"
              >
                Enroll & Start
              </button>
            </div>
